...
def create_app():
    ...
    @app.route('/api/super-admin/vouchers/<voucher_id>/email', methods=['POST'])
    @require_auth
    @require_role('super_admin')
    @log_access
    def super_admin_email_voucher(voucher_id):
        from src.application.super_admin_service import super_admin_service
        try:
            data = request.json or {}
            success = super_admin_service.send_voucher_email(voucher_id, data)
            if success:
                return jsonify({'success': True}), 200
            return jsonify({'success': False, 'error': 'Voucher not found or delivery failed'}), 400
        except Exception as e:
            logger.error(f"Error emailing voucher {voucher_id}: {e}")
            return jsonify({'success': False, 'error': str(e)}), 500

    @app.route('/api/super-admin/vouchers/analytics', methods=['GET'])
    @require_auth
    @require_role('super_admin')
    @log_access
    def super_admin_voucher_analytics():
        from src.application.super_admin_service import super_admin_service
        try:
            data = super_admin_service.get_voucher_analytics()
            return jsonify({'success': True, 'data': data}), 200
        except Exception as e:
            logger.error(f"Error fetching voucher analytics: {e}")
            return jsonify({'success': False, 'error': str(e)}), 500

    @app.route('/api/super-admin/vouchers/timeline', methods=['GET'])
    @require_auth
    @require_role('super_admin')
    @log_access
    def super_admin_voucher_timeline():
        from src.application.super_admin_service import super_admin_service
        try:
            data = super_admin_service.get_voucher_timeline()
            return jsonify({'success': True, 'data': data}), 200
        except Exception as e:
            logger.error(f"Error fetching voucher timeline: {e}")
            return jsonify({'success': False, 'error': str(e)}), 500
